...
export default function MyClasses() {
  ...
  const fetchEnrolledClasses = async () => {
    ...
    try {
      const res = await fetch(`http://class-management-backend.onrender.com/api/v1/classes/student/${studentId}`);
  ...
  const fetchAvailableClasses = async () => {
    ...
    try {
      const res = await fetch('http://class-management-backend.onrender.com/api/v1/classes/available', {
        headers: {
  ...
  const enrollInClass = async (classId) => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch('http://class-management-backend.onrender.com/api/v1/classes/enroll', {
        method: 'POST',
        headers: {
