...
export default function MyClasses() {
  ...
  const fetchEnrolledClasses = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/v1/classes/student/${studentId}`);
  ...
  const fetchAvailableClasses = async () => {
    ...
    try {
      const res = await fetch('http://localhost:3000/api/v1/classes/available', {
        headers: {
  ...
  const enrollInClass = async (classId) => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch('http://localhost:3000/api/v1/classes/enroll', {
        method: 'POST',
        headers: {
